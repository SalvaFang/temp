...
//    std::string kDir = "E:\3D-Eye-Tracker-master\Build\main/";
...
int main(int argc, char *argv[])
{
    // ✅ 必须添加：启用高 DPI 缩放 (Qt 5.14+)
    // 【关键】启用高 DPI 缩放（Qt 5.6+）
    #if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
        QApplication::setAttribute(Qt::AA_EnableHighDpiScaling);
        QApplication::setAttribute(Qt::AA_UseHighDpiPixmaps);
    #endif

    // 确保使用 OpenGL 或软件渲染时支持高 DPI
    #if QT_VERSION >= QT_VERSION_CHECK(5, 14, 0)
        QGuiApplication::setHighDpiScaleFactorRoundingPolicy(Qt::HighDpiScaleFactorRoundingPolicy::PassThrough);
    #endif

    QApplication a(argc, argv);
    a.setStyle(QStyleFactory::create("Fusion")); // 设置现代 Fusion 风格
    QFont font("Arial", 10); // 设置全局字体
    a.setFont(font);


   MainWindow w;
   w.setWindowTitle("增强目视-眼动跟踪");
   w.setWindowIcon(QIcon("eye.png"));
//   w.show();
