...
void getROI(const cv::Mat& src, cv::Mat& dst, const cv::Rect& roi, int borderType) {
    ...
    if (validROI == roi) {
        ...
        int top = max(-tl.y, 0);  // Top and left are negated because adding a border
        int left = max(-tl.x, 0); // goes "the wrong way"
        int right = max(br.x, 0);
        int bottom = max(br.y, 0);
...
float histKmeans(const cv::Mat_<float>& hist, int bin_min, int bin_max, int K, float init_centres[], cv::Mat_<uchar>& labels, cv::TermCriteria termCriteria) {
    ...
    while (true) {
        ...
        if (movedCount == 0)
            ...
            for (label = 0; label < K; ++label) {
                max_movement = max(max_movement, (float) std::pow(centres(label) - old_centres(label), 2));
            }
